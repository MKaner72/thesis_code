...
subplot(2,2,[3,3],'fontsize',FS)
rescale_factor = 5; % Adjust this factor as needed
grid on;

plot(z*1e6,n,'k','linewidth',LW)
...
plot(z*1e6,real(PSI(:,idx))*rescale_factor+1.5,'b.-','linewidth',LW)
plot(z*1e6,imag(PSI(:,idx))*rescale_factor+1.5,'g.-','linewidth',LW)
plot(z*1e6,(((abs(PSI(:,idx))).^2) * rescale_factor^2)+1.5,'r.-','linewidth',LW)

xlim([0 z(end)]*1e6)
ylim([-4 20])
title(strcat('@lambda=',num2str(lambda(idx)*1e9),'nm'))
xlabel('z (um)')
ylabel('Optical index / Electrical field (arbitrary units)')
legend('Refractive Index',"E (Re)","E (Im)",'|E|^2')
...
plot(lambda*1e9,T,'b-','linewidth',LW)
